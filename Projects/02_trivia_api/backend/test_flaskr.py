...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_get_questions_with_error(self):
    ...
    def test_get_questions_with_category_without_result(self):
        category_id="10"
        res = self.client().get('/categories/'+category_id+'/questions')
        data = json.loads(res.data)
    ...
    def test_random_question_game_with_notfound_error(self):
        res = self.client().post('/quizzes',json={'quiz_category':'10','previous_questions':'[]'})
        data= json.loads(res.data)
        self.assertEqual(res.status_code , 200)
