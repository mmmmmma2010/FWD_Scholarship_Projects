...
def test_previous(isprevious,previous_question,iscategory=None):
    if iscategory:
        if int(iscategory)>int((db.session.query(func.max(Category.id).label('max_category')).one()).max_category):
            abort(404)
        ...
        random_num =int(random.choice(db.session.query(Question.id).all())[0]) 
        while isprevious:
            for q in previous_question:
                if random_num == q:
                    isprevious=True
